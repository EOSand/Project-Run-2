...
execute at @p if block ~ ~-0.25 ~ minecraft:spruce_planks if entity @p[y=74.0,distance=..0.25] unless entity @p[tag=Library_CHECK1] unless entity @p[tag=Library_CHECK2] run tp @p -1780.5 83.0 824.5 -180 0
...
execute if entity @p[nbt={Inventory:[{id:"minecraft:enchanted_book",Count:1b,tag:{display:{Name:'{"text":"Tome of Monscollis","italic":false}'}}}]}] unless entity @p[tag=Library_DONE] run tag @p add Library_DONE 
...
execute if entity @p[tag=Library_DONE] if score @p Timer matches 70 run tp @p -1780.5 83.0 824.5 -180 0
